...
void getUniversalSet() {
    ...
    if (superSetSize > MAX_SIZE) {
        printf("Error: Size exceeds maximum limit of %d.\n", MAX_SIZE);
        exit(1);
    }
...
void getSet(int arr[], int *size) {
    printf("Enter %d elements for the set (must be in the Universal Set):\n", *size);
...
int main() {
    ...
    do {
        printf("\nEnter Set A Size (max %d): ", superSetSize);
        scanf("%d", &setASize);
    ...
    do {
        printf("\nEnter Set B Size (max %d): ", superSetSize);
        scanf("%d", &setBSize);
